...
| Unified citation-ready memory and resource evidence | bounded `search` command and stable citation fields | `test_unified_search_returns_cited_memory_and_neighbor_expanded_resource_evidence` |
| Long-document context survives section matching | heading-scoped index plus previous/next section expansion | same unified-search test |
| Rare exact terms influence ranking without a model | field weighting plus inverse document frequency | retrieval budget test and unified-search score assertions |
| Unified search never hides a selected conflict | bounded companion expansion beyond the requested limit | `test_conflicts_core_relevance_and_unrelated_omission` |
...
| Rejected automatic-policy push cannot rebase over a concurrent revocation | fetched-history recheck plus immutable local supersession before retry | `test_owner_policy_push_race_does_not_override_concurrent_revocation` |
| Installed owner resource commands recover private but not sensitive content | shared attested owner-local context for memory and resource reads | `test_installed_owner_cli_reads_private_resources_but_source_invocation_fails_closed` |
...
| Cold/warm behavior at 30/1k/10k records | synthetic mixed-corpus benchmark | `megabrain.py benchmark`; [retrieval activation report](benchmarks/retrieval-activation-2026-08-27.md) |
