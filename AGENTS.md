...
- Retrieval benchmark: `python3 skill/megabrain/scripts/megabrain.py benchmark`
- Vault tests: `python3 -m unittest tests.test_vault -v`
- Vault dependency audit: `python3 -m pip check`
- Validate skill with the `skill-creator` quick validator when it is available.

## Boundaries

- The Brain runtime uses the Python standard library and Git. Vault may additionally use only the pinned PyNaCl range and Python `sqlite3`.
- Do not introduce a network server, hosted relay, autonomous crawler, or unrelated package dependency. Vault's optional broker is same-host Unix-socket only.
...
- Vault setup may return recovery material exactly once through its explicit setup response. No other command, log, test failure, browser, or doctor report may expose it.
