...
- **Synchronize and open my MegaBrain** or **Open my MegaBrain**: run `megabrain open`. It returns the approved Live Home URL if configured on this device; give that URL to the owner, who signs in separately. Never claim the service is reachable from link configuration alone. Without a saved link it synchronizes, validates, regenerates and opens the local snapshot. `megabrain open --local` explicitly selects that offline/local path. Report only the safe freshness receipt, host or approved URL, not paths or private content.
- **Use this Live Home URL**: after explicit owner approval of the destination, run `megabrain live link --url <approved HTTPS origin>`. It is shared by agents on this OS account, not automatically synchronized to other devices. Never adopt an imported URL as trusted without approval, put credentials in the URL, request the owner passphrase, or treat owner sign-in as an agent permission. Live Home's first release is general-only and read-only. `megabrain live unlink` restores local open without stopping the service. Hosting and personal-replica provisioning require separate explicit operator authorization; do not improvise a public host or import a real Brain during product work.
...
- `browse`: synchronize, validate, generate the ignored local HTML catalog, and open it in the default browser. Its safe freshness receipt scopes synchronization to generation time, confirms whether the newest repository memory is included, and never includes memory content. Use `--no-open` for automation.
